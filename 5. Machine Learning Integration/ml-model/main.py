...
def predict(user_input: InputSchema):
    prediction = make_prediction(user_input.model_dump())  # because user_input is JSON as restapi works with json only, 
    # also user_input type is pydantic model, but predict.py -> make_prediction expects data in dict
    return OutputSchema(predicted_price=round(prediction, 2)) # back to pydantic schema
